...
if si_xn != si_x:
  ...
  for nz in range(0,si_z):
    fint = RegularGridInterpolator((yy, xx),uvel[nz,:,:], method='linear')
    uvel_t[nz,:,:] = fint((yng,xng))
  
    fint = RegularGridInterpolator((yy, xx),vvel[nz,:,:], method='linear')
    vvel_t[nz,:,:] = fint((yng,xng))
  
    fint = RegularGridInterpolator((yy, xx),theta2[nz,:,:], method='linear')
